        ...
        $configFixtureString = @"
{
    ...
    "Host": {
        ...
        "esxsyslog": [
            "udp://loginsight.corp.com:514"
        ],
        "esxsyslogfirewallexception": true,
        "iovdisableir": "FALSE",
...
"@
        ...
        try {
            Write-Warning -Message "[$($PSCmdlet.MyInvocation.MyCommand.Name)] Creating Basic Config file { $outputFolder\Config.json }. Customizations of this file will need to be manually made. "
            $configFixtureString | Out-File "$OutputFolder\Config.json" -Encoding UTF8 -ErrorAction 'stop'
        } catch {
            Write-Warning -Message "[$($PSCmdlet.MyInvocation.MyCommand.Name)][ERROR] Could not create the Config file { $outputFolder\Config.json }. Continuing without Config file"
        } # try/catch
